...
коммит `ad3ab9dabcb2661853d232c355e58f72ec36c0e9`.
...
**Общая память трактовок должна быть явным компонентом.** В контекстной
схеме разные преобразования входа обращаются к согласованному опыту.
Независимые памяти, обученные на разных парах меток, ещё не образуют такой
механизм. В первом приближении нужны общий опыт, локальные преобразователи
и общее пространство сравнения.
...
| Общая память трактовок для разных контекстов | Не реализована как цельный контекстный механизм |
...
компонентов обозначают обязанности, не готовые библиотеки.
...
| `SharedInterpretationMemory` | Сравнивать кандидатов с согласованным опытом; использовать факторное извлечение и сохранять основания |
...
| `LanguageModel` | Связывать форму выражения со структурой смысла и строить выражение по выбранному содержанию |
...
Здесь `LanguageModel` — будущий собственный языковой компонент AI2,
а не предположение о подключении готового собеседника. Его устройство
нужно разработать и проверить. На ранних ступенях явный парсер и шаблоны
...
| 3. Общая память и заданные контексты | Несколько преобразователей к согласованному представлению и общий механизм сравнения | Опыт одного вида помогает узнать тот же объект/связь в другом, сохраняя различия |
...
### Этапы 3–4: реализовать именно контекстное узнавание

Сначала допустимы заданные виды и реальные пары одного события в разных
видах. Из них обучаются преобразователи к фиксированному общему коду.
Единый код на этой ступени задаётся разработчиком — это ещё не открытие
системы понятий. Проверяем отсутствие схлопывания разных объектов,
сохранение существенного содержания и перенос на новые сочетания.
...
| `Observation` | Измеренные поля, полнота области, источник, ID измерения и физического перехода |
...
|---|---|---|
| Смысловое представление | Набор активных битов не задаёт участников, время, роли и область утверждения | Связанное содержание с происхождением; SDR как обучаемое основание и индекс |
...
| Общая память | Отдельные банки отображений не обеспечивают общую проверку трактовок | Согласованный опыт, версия общего кода и проверяемая маршрутизация |
...
| B. Контекстный срез | Пара видов, обучаемые преобразователи, общая память и проверяемые факторные основания | Один и тот же небольшой опыт используется при разных трактовках |
