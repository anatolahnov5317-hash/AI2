...
После строгой проверки соответствия язык `if/copy` оставлен дополнительным
стендом. Он не является обязательным следующим слоем ядра. Приоритет —
развитие уже имеющегося контекстного узнавания, его нескольких результатов
и обучения преобразованиям. Записи участников и ролей ниже относятся
к учебной среде и не устанавливают универсальный формат всякого смысла.

Цель дополнительного эксперимента — научиться собирать из факторных свидетельств несколько
...
В рассматриваемой учебной среде ситуация представляется набором частичных объяснений. В каждом объяснении
...
предполагается. При запросе действия добавляется прогноз его наблюдаемого
последствия; обычное узнавание не требует действия или оператора динамики.
Разные узнанные части могут сосуществовать; только несовместимые трактовки
образуют взаимоисключающие варианты.

Схема ниже показывает случай с действием, а не обязательный маршрут
каждого запроса к памяти.
...
Контекстные трактовки одной области/уровня сопоставляются с согласованной
памятью опыта. Другие области могут иметь свои коды; результат сохраняет
и узнанное содержание, и контекст. Узкий вариант уже работает в
[`TextFactorModel._select_context`](../src/text_factors/model.py): все
заданные сдвиги сравниваются с одной памятью, затем выбирается один.
Независимые банки `ExperienceMemory` — другой экспериментальный маршрут,
поэтому их устройство не доказывает отсутствия общей памяти в AI2.
...
Это формат адаптера и диагностики учебного мира; он не требует отдельного
графового решателя и не заменяет распределённое обучаемое представление.
...
| `Explanation` | Контекст, привязки и содержание; кандидаты действий/правил только для соответствующего запроса; подтверждения, противоречия, неизвестное | Допустимы совместные части и несовместимые варианты; итоговый балл не заменяет их содержание |
...
## 5. Дополнительный эксперимент: явные правила из эпизодов

Ниже описана наша проверяемая гипотеза об индукции коротких программ.
Она не обязательна для прямого обучения контекстным преобразованиям
разреженных кодов и не приписывается Редозубову. Язык эффектов `if/copy`
также не является упомянутой автором рекуррентной языковой грамматикой.

В этом эксперименте правила появляются в два шага: локальное факторное свидетельство выделяет
...
### Ограниченный язык этого стенда
...
В этом вертикальном срезе — одна изменяемая величина и эффект глубины 1.
...
### Проверка факторного вклада в этом режиме

В эксперименте по факторному обобщению каждое применённое обученное правило должно иметь проверяемую связь
...
Это ограничение данного режима, а не запрет других видов узнавания во
всей модели. В
[части 5, разделе о функции соответствия](https://habr.com/ru/articles/309626/)
автор допускает точное, приближённое, факторное и комбинированное сравнение.
Узнавание сохранённого случая не обязано ждать выделения устойчивого
фактора. При использовании нескольких путей их вклад измеряется отдельно;
прямое запоминание или готовый ответ не называется факторным обобщением.
...
Перед конкуренцией нужно различить совместимые части описания, близкие
дубли одного узнавания и несовместимые трактовки. Два присутствующих
объекта не должны вытеснять друг друга как две версии одного объекта.
В [части 7, «Коррелированные контексты»](https://habr.com/ru/articles/310960/)
рассмотрен отбор нескольких максимумов с подавлением близких откликов.
Ранжирование вариантов без учёта этих отношений недостаточно.
...
новую запись к тому же действительному событию. До раскрытия объяснение
может иметь прогноз, но этот прогноз не заполняет целевую пару обучения.
Тип события указан явно: действие среды, измерение, сообщение. В данном
стенде исследуются физические переходы; не всякое преобразование `до/после`
в теории ограничивается ими. Внутренняя гипотеза не становится новым
внешним событием при таком расширении типов.

Журнал различает:

- `transition_id`: одно действительное событие указанного типа; в физическом стенде — выполнение действия;
...
Строгая сверка уточняет приоритет: сначала расширить уже существующее
контекстное узнавание, возвращать несколько содержательных результатов
с контекстами и исследовать обучение преобразованиям. Для этого не
обязательны полный граф участников и генератор символьных программ.
Таблица ниже относится к отдельному эксперименту §5, если он будет выбран
для проверки; это не обязательная последовательность построения всей AI2.
...
Исполняемый срез этого дополнительного эксперимента должен пройти один
небольшой мир целиком. В его объём не входят автоматическое выделение
объектов, открытие произвольных контекстов, обучение с масками внутри ядра,
композиция операторов, полноценное планирование и язык. Развитие этих
механизмов в основной архитектуре не зависит от завершения данного стенда.
...
Временная ориентация для этого эксперимента: контракты → один исполняемый
...
| Согласование центра и периферии | `гиль(1).txt`, 487–488, 604; `Новый текстовый документ(1).txt`, 4219 | Согласование локальных кодов; версионированный ориентир — один из возможных инструментов |
